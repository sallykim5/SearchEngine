...
//3. Relevance is stored as one of the variables in the Page object, which can be used by quicksort to rank the pages by
...
public class Page implements Comparable<Page>{
    ...
    public static void setRel(Page p, String [] query) { //set relevance value to each page
    	if(query.length==1)
    ...
    public int compareTo(Page other) { //compares the relevance of one page to another
    ...
    public static boolean remove(LinkedList<Page> l, Page p){ //removes pages in collected linked list that have the same length of text (same pages)
    	if(l.isEmpty())
    		return false;
    	Iterator<Page> remove = l.iterator(); 
		while(remove.hasNext()){
			Page next = remove.next();
    ...
    public static double relev(Page p, String [] query){ //calculating relevance value when there are more than one query word
    	...
    	String strQuery = "";
    	if(query.length>1){ //if query is more than one word, it is more relevant if text includes the entire query, not just part of it
	    	for(int i=0; i<query.length; i++){
	    		if(i == query.length-1)
	    			strQuery+= query[i];
	    		else
	    	...
	    	int index1 = txt.indexOf(strQuery);
	    	while(index1!= -1){
	    		count++; //count increases by 1 when it finds entire query 
	    		txt = txt.substring(index1+1);
	    		index1 = txt.indexOf(strQuery);
	    	}
	    }
    	for(int i=0; i<query.length; i++){ 
    		int index = txt.indexOf(query[i]); //finds the first index of each query word in the text
	    	while(index!= -1) {
	    		count+=0.95; //count increases by 0.95 when it finds one word of the query
	    		txt = txt.substring(index + 1); 
	    		index = txt.indexOf(query[i]); //finds index of where query word next appears
	    	}
	    }
    ...
    public static double relev2(Page p, String [] query){ //calculating relevance value when there is only one relevance value (if we use same relev() method as that of when there are multiple query words => read timed out error)
    	int count = 0; 
    	double relev = 0;
    	double total = p.getText().length();
    	String txt = p.getText().toLowerCase();
   	...
   	public static int getQIndex(Page p, String [] query) {	
    ...
    public static boolean isSpace(Page p, int index){ //to make sure that when printing snippet of text, it does not start/end in the middle of a word
    ...
    public static void finalResult(Page [] result, String [] query){
		boolean print = true;
    ...
    public static void main(String[] args) throws IOException {
    	LinkedList<String> frontier = new LinkedList<String>(); //linkedlist of all the pages 
    	LinkedList<Page> collected = new LinkedList<Page>(); //linkedlist of all the collected pages that have query words 
		LinkedList<String> visited = new LinkedList<String>(); //linkedlist of all the pages that have been visited / iterated through their adjacent pages
		...
		Page p = new Page(args[0].substring(2, args[0].length()));
		...
		while(true){
			...
			try {	    	
				...
				for (String w: p.adjacentURL()) 
			    	frontier.addLast(w);
			    if(collected.size()==num){ //once we have enough pages in collected linkedlist 
			    	Page[] find = new Page[num];
			    	for(int i=0; i<num; i++)
